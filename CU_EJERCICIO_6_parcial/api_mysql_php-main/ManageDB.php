...
class ManageBD extends DB{
  public function getQueries(){
    ...
    $reunionnatural = $this->connect()->query('SELECT DISTINCT c.*, t.* FROM course AS c NATURAL JOIN takes AS t');
    ...
    ");
    $json = $this->connect()->query('SELECT * FROM json_all WHERE id=3');
    $reunionnatural2 = $this->connect()->query('SELECT DISTINCT c.*, t.* FROM course AS c NATURAL JOIN takes AS t');
